...
public class TypeTemplateServiceImpl implements TypeTemplateService {
	...
	private void saveToRedis(){
		...
		for (TbTypeTemplate typeTemplate : all) {
			...
			redisTemplate.boundHashOps("brandIds").put(typeTemplate.getId(), brandList);

			//缓存规格列表
			List<Map> specList = findSpecList(typeTemplate.getId());
			redisTemplate.boundHashOps("specIds").put(typeTemplate.getId(), specList);
		}
	}
    ...
    public List<Map> findSpecList(Long id) {
		...
		//把数据库查询出来的规格名称json串，转换为List<map>格式
		List<Map> result = JSON.parseArray(typeTemplate.getSpecIds(), Map.class);
		//跟据规格名称id查询规格选项列表
		for (Map map : result) {
			//查询规格选项列表
			Long specId = new Long(map.get("id").toString());
			TbSpecificationOption where = new TbSpecificationOption();
			where.setSpecId(specId);
